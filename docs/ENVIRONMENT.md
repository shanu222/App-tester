...
Vercel Cron sends `Authorization: Bearer $CRON_SECRET` to `/api/cron/tick` and `/api/cron/daily-testing-summary` (`0 11 * * *` ≈ 4:00 PM Asia/Karachi). Each digest run honors the developer’s saved frequency, time, weekday, and timezone and does not send duplicates.
